...
for n,d in [(51,64),(251,3)]:
    ...
    for fold in range(3):
        ...
        for lam in [1e-4,1e-3,1e-2,.1,1,10,100]:
            begin=time.perf_counter()
            z,dz,diag=smooth_tv(y,t,lam,mask=mask,tol=1e-4,max_iter=10000)
            assert diag['max_relative_gap']<=1e-4
            assert diag['dual_box_violation']==0
...
Path(__file__).with_name('validation_report.json').write_text(json.dumps(report,indent=2)+'\n')
